...
너는 한국어로 진행하는 시니어 면접관 겸 면접 코치다.
사용자의 목표 직무와 경력에 맞춰 실제 면접처럼 질문하되, 답변 뒤에는 구체적이고 행동 가능한 피드백을 제공한다.
...
반드시 유효한 JSON 객체 하나만 출력한다. 마크다운 코드펜스를 쓰지 않는다.
`;

const promptFor = ({ action, profile, transcript, history, telemetry, questionNumber, totalQuestions, sessionMetrics }) => {
  if (action === 'start') {
    return {
      task: '첫 면접 질문 생성',
      profile,
      ...
      rules: [
        '첫 질문은 자기소개 또는 지원동기 중 프로필에 더 적합한 것으로 시작한다.',
        '질문은 한 번에 하나만 한다.',
        '질문은 2문장 이내로 작성한다.',
      ],
    };
  ...
  if (action === 'answer') {
    return {
      ...
      rules: [
        ...
        'betterAnswer는 사용자의 실제 답변과 프로필을 바탕으로 5-8문장으로 재구성한다.',
  ...
  return {
    ...
    rules: [
      '각 우선과제는 연습 방법까지 포함한다.',
      '7일 계획은 매일 수행 가능한 단위로 7개를 작성한다.',
      '합격을 보장하는 표현은 쓰지 않는다.',
    ],
  };
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sessionMetrics = cleanTelemetry(body.sessionMetrics);
    const questionNumber = Math.max(1, Math.min(20, Number(body.questionNumber) || 1));
    const totalQuestions = Math.max(1, Math.min(20, Number(body.totalQuestions) || 5));

    if (action === 'answer' && transcript.length < 2) {
      return res.status(400).json({ error: '평가할 답변이 없습니다.' });
    }
    ...
    const parsed = parseJson(outputTextFromResponse(data));
    return res.status(200).json(parsed);
  } catch (error) {
    return res.status(500).json({ error: error?.message || '면접 코치 처리 중 오류가 발생했습니다.' });
  }
}
